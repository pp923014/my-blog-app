...
// Logout function: Clears the JWT token from cookies
const logout = (req, res) => {
  try {
    // Clear the cookie by setting the token to empty and expiring it immediately
    res.cookie("token", "", {
      httpOnly: true,
      expires: new Date(0), // Set the cookie expiration to a past date to clear it
      secure: true,
    });
    return res.status(200).json({
      success: true,
